...
<div id="div-info" class="alert alert-info" style="margin-top:10px; display:none;">
    <!--Text-->
</div>
...
<div class="table-responsive">
    ...
    <div id="div-button-action">
        <c:if test="${showSendButton==true}">
            <button type="button" class="btn btn-primary btn-sm send-all-btn">Send All</button>
        </c:if>
    </div>
</div>
...
<div id="parameter-awal" style="display:none;">
    ...
    <input class="all-checked-status" value="1" />
...
<script>
    $(document).ready(function(){
        ...
        document.title = "Validasi Statement";
        ...
        loadPagination(cifKey, accountNumber, periode, status);
    ...
    function openFile(sendEmailId){
    	...
    	var ajax = $.ajax({
    		url: "<c:url value='send-email/open-file?id="+sendEmailId+"&isReprint=0' />",
    		type: 'GET'
    	});
    ...
    function checkAllEvent(){
        $("#parent-check").on("change", function(){
            if($(this).is(":checked")){
                $(".email-checkbox:enabled").prop("checked", true);
    ...
    function refreshTable(cifKey, accountNumber, periode, status, pageNumber, totalPerPage, checkedStatus){
        var ajax = $.ajax({
                        url: "<c:url value='/send-email/list?cifKey="+cifKey+ "&accountNumber="+accountNumber+ "&periode="+periode+"&status="+status+"&current="+pageNumber+"&size="+totalPerPage+"' />",
                        timeout : 30000,
                        type : 'get'
                    });
        ...
        ajax.done(function (response) {
            if (response.resultCode == 1000) {
                ...
                $.each(response.data, function(i,item){
                	var accNo = item.accountNumber!=null ? item.accountNumber : "N/A";
                    if( accNo.length == 19 )
                	...
                	if(checkedStatus==1){
                        ...
                        row += (item.cifKey!=null ? item.cifKey : "N/A") + "</td><td style='text-align:left;'>"
	                            + accNo + "</td><td style='text-align:left;'>"
    ...
    function loadPagination(cifKey, accountNumber, periode, status){
       var ajax2 = $.ajax({
            url: "<c:url value='/send-email/get-row-whitelist?cifKey="+cifKey+"&accountNumber="+accountNumber+"&periode="+periode+"&status="+status+"' />",
            timeout : 30000,
            type : 'get'
        });
		...
		if (response.resultCode == 1000) {
		    ...
		    var ajax = $.ajax({
                url: "<c:url value='/send-email/get-row?cifKey="+cifKey+"&accountNumber="+accountNumber+"&periode="+periode+"&status="+status+"' />",
                timeout : 30000,
                type : 'get'
            });
			ajax.done(function (response) {
			    if (response.resultCode == 1000) {
			        refreshPagination(response.data);
			        $(".old-total-records").val(response.data);
			        if(response.data==0){
			        	hideDivInfo();
			        }
			        refreshTable(cifKey, accountNumber, periode, status, 1, $(".old-total-per-page").val(), getAllCheckedStatus());
			    }
			    else {
			        alert(response.message);
			    }
    ...
    function setActivePage(pageNumber){
        ...
        $.each($(".page-number-btn-pagination"), function(){
            ...
            var currentPage = parseInt($(this).children().first().val());
            if(currentPage == pageNumber)
                $(this).addClass("active");
        })
    }

    function setOldValues(cifKey, accountNumber, periode, status){
        $(".old-cif-key").val(cifKey);
        $(".old-account-number").val(accountNumber);
    ...
    function searchButtonEvent(){
        $("#search-btn").off().on("click", function(){
            ...
            refreshTable(cifKey, accountNumber, periode, status, 1, totalPerPage, getAllCheckedStatus());
            loadPagination(cifKey, accountNumber, periode, status);

            setOldValues(cifKey, accountNumber, periode, status);
            hideDivInfo();
            showPagination();
        })
    }
    ...
    function resetAllChecked(){
        ...
        $("button.send-all-btn").text("Send Email");
    }
    ...
    function selectAllData(total){
        ...
        $("button.send-all-btn").text("Send All");
    ...
    function ajaxRequestSendEmail(actionString){
        if(getAllCheckedStatus()==1){
            ...
            if(actionString=="send"){
                actionUrl = "<c:url value='/send-email/send-all' />";
            } else {
                actionUrl = "<c:url value='/send-email/resend-all' />";
            }
            ...
            var actionUrl;
            if(actionString=="send"){
                actionUrl = "<c:url value='/send-email/send' />";
            } else {
                actionUrl = "<c:url value='/send-email/resend' />";
            }
