...
import com.optima.nisp.response.KonsolidasiEmailLogResponse;
...
import com.optima.nisp.service.KonsolidasiEmailService;
...
@Controller
@RequestMapping(value="konsolidasi-email")
@SessionAttributes("userSession")
public class KonsolidasiEmailController {

	private static final Logger logger = Logger.getLogger(KonsolidasiEmailController.class);
	
	@Autowired
	private KonsolidasiEmailService konsolidasiService;
	...
	public ModelAndView getManageSendEmailPage(Model model, HttpServletRequest request){
		ModelAndView mv = new ModelAndView("monitoring-konsolidasi");
		try{
			if(model.containsAttribute("userSession")){
				...
				Button resendEmailBtn = buttonService.getByButtonUrl("konsolidasi-email/resend");
	...
	public ModelAndView getValidasiPage(Model model, HttpServletRequest request){
		ModelAndView mv = new ModelAndView("validasi-konsolidasi");
		try{
			if(model.containsAttribute("userSession")){
				UserSession userSession = (UserSession) model.asMap().get("userSession");
				Long[] sessionGroupIds = userSession.getGroupIds();
				
				Button sendEmailBtn = buttonService.getByButtonUrl("konsolidasi-email/send"); 
	...
	public Response getListEmail(Model model, 
		...
		try{
			...
			if(model.containsAttribute("userSession")){
				List<KonsolidasiEmailResponse> emails = konsolidasiService.getEmails(cifKey, periode, status, pageNumber, totalPerPage);
	...
	public Response getTotalRow(Model model, 
		...
		try{
			...
			if(model.containsAttribute("userSession")){
				int totalRow = konsolidasiService.getTotalRow(cifKey, periode, status);
				resp.setResultCode(1000);
				resp.setMessage("Sukses get data");
	...
	public Response getTotalRowWhitelist(Model model, 
			@RequestParam("cifKey") String cifKey,
			@RequestParam("periode") String periode, 
			@RequestParam("status") int status){
		try{
			...
			if(model.containsAttribute("userSession")){
				int totalRow = konsolidasiService.getWhitelistTotalRow(cifKey, periode, status);
				resp.setResultCode(1000);
				resp.setMessage("Sukses get data");
				resp.setData(totalRow);
			} else {
	...
	public Response send(Model model, @RequestBody Long[] listId){
		try{
			...
			if(model.containsAttribute("userSession")){			
				List<KonsolidasiEmail> emails = new ArrayList<KonsolidasiEmail>();
//				
//				for(Long id : listId){
...
//				sendEmailService.sendAll(emails);
				konsolidasiService.addToQueue(listId);
				konsolidasiService.sendNextQueue();
	...
	public Response sendAll(Model model, HttpServletRequest request, @RequestBody KonsolidasiEmailResponse sendEmail){
		try{
			Response resp = new Response();
			
			if(model.containsAttribute("userSession")){			
				List<KonsolidasiEmail> emails = konsolidasiService.getEmails(sendEmail.getCifKey(), 
						sendEmail.getAccountNumber(), sendEmail.getPeriode(), sendEmail.getStatus());
			Long[] listId = new Long[emails.size()];
			int i=0;
			for (KonsolidasiEmail se : emails) {
				listId[i] = se.getId();
				i++;
			}
			konsolidasiService.addToQueue(listId);
	...
	public Response resendAll(Model model, HttpServletRequest request, @RequestBody KonsolidasiEmailResponse sendEmail){		
	...
	public ModelAndView getSendEmailLogPage(Model model, HttpServletRequest request){
		...
		try{
			if(model.containsAttribute("userSession")){
				mv.setViewName("konsolidasi-email-log");
	...
	public Response getLogList(Model model, HttpServletRequest request, @RequestParam int start, 
		...
		try{
			...
			if(model.containsAttribute("userSession")){			
				List <KonsolidasiEmailLog> logs = konsolidasiService.getLogList(subject, mailTo, tglKirimFrom, tglKirimTo, status, start, length);
				List <KonsolidasiEmailLogResponse> logsResponse = new ArrayList<KonsolidasiEmailLogResponse>();
				Integer totalRow = konsolidasiService.getTotalLogRow(subject, mailTo, tglKirimFrom, tglKirimTo, status);
				int number = 1;
				
				for(KonsolidasiEmailLog log : logs){
					KonsolidasiEmailLogResponse logResp = new KonsolidasiEmailLogResponse();
	...
	public Response openFile(Model model, @RequestParam("id") Long id, @RequestParam int isReprint){		
		try{
			...
			if(model.containsAttribute("userSession")){			
				...
				KonsolidasiEmail sendEmail = konsolidasiService.getById(id);
