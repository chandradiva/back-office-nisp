...
@Component
public class SendEmailLogDao extends ParentDao<SendEmailLog> {

	public List<SendEmailLog> getList(String subject, String mailTo, String tglKirimFrom, String tglKirimTo, int status, int start, int length) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, InstantiationException, NoSuchMethodException, SecurityException{	
		...
		String sql = "SELECT * FROM (SELECT Q.*, ROWNUM R FROM ( SELECT * FROM "+CommonCons.DB_SCHEMA+".BO_TBL_SEND_EMAIL_LOG "
		...
		List<SendEmailLog> res = new ArrayList<SendEmailLog>();

		List<Map<String, Object>> rows = template.queryForList(sql);
		for (Map<String, Object> row : rows) {
			SendEmailLog log = serialize(SendEmailLog.class, row);
			res.add(log);
		}
		return res;
	}
	...
	public Integer getTotalRow(String subject, String mailTo, String tglKirimFrom, String tglKirimTo, int status) {
		...
		String sql = "SELECT COUNT(*) TOTAL FROM ( SELECT * FROM "+CommonCons.DB_SCHEMA+".BO_TBL_SEND_EMAIL_LOG "
				+ "WHERE LOWER(MAIL_TO) LIKE '%"+mailTo+"%' " + sqlStatus + sqlSubject + sqlTglFrom + sqlTglTo
				+ "ORDER BY TIME DESC)";
